...
def plotting_tool_rula(time, score, base_score, conf, label, file):
    ...
    ax0.plot(time, score, '-o', drawstyle='steps-post', lw=2, ms=4, color=blue, label="Mit Unsicherheit")    #plot with steps
    ax0.plot(time, base_score, '--', drawstyle='steps-post', lw=2, color=red, label="Ohne Unsicherheit")    #plot with steps
    ...
    ax1.plot(time, conf, drawstyle='steps-post', lw=2, color=palette[7])    #plot with steps
...
def plot_rula():
    ...
    rula = np.loadtxt("../data/avg_100.csv", delimiter=' ', skiprows=1)      #load data from path and skip first row that contains entry names
    time = rula[:,0]
