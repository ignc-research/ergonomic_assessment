...
def neck_twist_score(angle):
    """is neck twisted?"""
    if ((angle < 45) or (135 < angle)):     #dguv werte
        return 1
    else:
        return 0
...
def neck_bend_score(angle):
    ...
    if ((angle < 80) or (100 < angle)):     #dguv werte = eigene werte
        return 1
    else:
        return 0
...
def abduction_score(angle):
    """is arm abducted?"""
    if (angle < 110):     #dguv werte (rest position + 20)
        return 0
    else:
        return 1
