...
def table_a(lower_arm, wrist, upper_arm):
    ...
    #print("upper:{} lower:{} wrist:{}".format(upper_arm, lower_arm, wrist))
    table = [
...
# def leg_score_adj(angle):
#     """adjust score based on angle of leg"""
#     if (30 < angle <= 60):
#         return 1
#     elif (60 < angle):
#         return 2
...
def lower_arm_evaluation(elbow_left, elbow_right, shoulder_left, shoulder_right, wrist_left, wrist_right):
    ...
    #print("L: {} {} R: {} {}".format(lower_arm_l.score, midline[0].score, lower_arm_r.score, midline[1].score))
    return [score_l, score_r]
...
def wrist_evaluation(elbow_left, elbow_right, wrist_left, wrist_right, hand_left, hand_right):
    ...
    #print("L: {} {} R: {} {}".format(wrist_l_angle.angle, wrist_l.score, wrist_r_angle.angle, wrist_r.score))
...
class Ergonomy():
    def __init__(self):
        ...
        header = np.array(["time", "rula_score", "rula_base_score", "rula_confidence",
                                    ...
                                    "legs_score", "legs_base_score", " legs_confidence",
                                    "upper_arm_left_score", "upper_arm_left_base_score",
                                    "upper_arm_right_score", "upper_arm_right_base_score", "upper_arm_right_confidence",
                                    "lower_arm_left_score", "lower_arm_left_base_score", "lower_arm_left_confidence",
    ...
    #     if not os.path.exists("rula_data"):     #check if target folder already exists
    #         os.makedirs("rula_data")
    #     rula = np.array([time, self.rula_score.score, self.rula_score.base_score, self.rula_score.confidence])
    #     with open('rula_data/rula_scores.csv', 'a') as f:
    #         np.savetxt(f, [rula], fmt='%1.8f', delimiter=' ')
    #     neck = np.array([time, self.neck.score, self.neck.base_score, self.neck.confidence])
    #     with open('rula_data/neck.csv', 'a') as f:
    #         np.savetxt(f, [neck], fmt='%1.8f', delimiter=' ')
    #     trunk = np.array([time, self.trunk.score, self.trunk.base_score, self.trunk.confidence])
    #     with open('rula_data/trunk.csv', 'a') as f:
    #         np.savetxt(f, [trunk], fmt='%1.8f', delimiter=' ')
    #     legs = np.array([time, self.legs.score, self.legs.base_score, self.legs.confidence])
    #     with open('rula_data/legs.csv', 'a') as f:
    #         np.savetxt(f, [legs], fmt='%1.8f', delimiter=' ')
    #     upper_left = np.array([time, self.upper_arm_left.score, self.upper_arm_left.base_score, self.upper_arm_left.confidence])
    #     with open('rula_data/upper_arm_left.csv', 'a') as f:
    #         np.savetxt(f, [upper_left], fmt='%1.8f', delimiter=' ')
    #     upper_right = np.array([time, self.upper_arm_right.score, self.upper_arm_right.base_score, self.upper_arm_right.confidence])
    #     with open('rula_data/upper_arm_right.csv', 'a') as f:
    #         np.savetxt(f, [upper_right], fmt='%1.8f', delimiter=' ')
    #     lower_left = np.array([time, self.lower_arm_left.score, self.lower_arm_left.base_score, self.lower_arm_left.confidence])
    #     with open('rula_data/lower_arm_left.csv', 'a') as f:
    #         np.savetxt(f, [lower_left], fmt='%1.8f', delimiter=' ')
    #     lower_right = np.array([time, self.lower_arm_right.score, self.lower_arm_right.base_score, self.lower_arm_right.confidence])
    #     with open('rula_data/lower_arm_right.csv', 'a') as f:
    #         np.savetxt(f, [lower_right], fmt='%1.8f', delimiter=' ')
    #     wrist_left = np.array([time, self.wrist_left.score, self.wrist_left.base_score, self.wrist_left.confidence])
    #     with open('rula_data/wrist_left.csv', 'a') as f:
    #         np.savetxt(f, [wrist_left], fmt='%1.8f', delimiter=' ')
    #     wrist_right = np.array([time, self.wrist_right.score, self.wrist_right.base_score, self.wrist_right.confidence])
    #     with open('rula_data/wrist_right.csv', 'a') as f:
    #         np.savetxt(f, [wrist_right], fmt='%1.8f', delimiter=' ')        
    ...
    def callback(self, data):
        if (data.markers):
            ...
            #print(time)
            rula = np.array([time, self.rula_score.score, self.rula_score.base_score, self.rula_score.confidence,
                                    self.neck.score, self.neck.base_score, self.neck.confidence,
                                    self.trunk.score, self.trunk.base_score, self.trunk.confidence,
                                    self.legs.score, self.legs.base_score, self.legs.confidence,
                                    self.upper_arm_left.score, self.upper_arm_left.base_score, self.upper_arm_left.confidence,
                                    self.upper_arm_right.score, self.upper_arm_right.base_score, self.upper_arm_right.confidence,
                                    self.lower_arm_left.score, self.lower_arm_left.base_score, self.lower_arm_left.confidence,
                                    self.lower_arm_right.score, self.lower_arm_right.base_score, self.lower_arm_right.confidence,
                                    self.wrist_left.score, self.wrist_left.base_score, self.wrist_left.confidence,
                                    self.wrist_right.score, self.wrist_right.base_score, self.wrist_right.confidence])

            angles = np.array([time,
                                    self.neck.angle,
                                    self.trunk.angle,
                                    self.legs.angle,
                                    self.upper_arm_left.angle,
                                    self.upper_arm_right.angle,
                                    self.lower_arm_left.angle,
                                    self.lower_arm_right.angle,
                                    self.wrist_left.angle,
                                    self.wrist_right.angle])

            with open(self.filename, 'a') as f:
                np.savetxt(f, [rula], fmt='%1.8f', delimiter=' ')

            with open("angles.csv", 'a') as data:
                #header=#time; neck_angle; trunk_angle; legs_angle; upper_arm_left_angle; upper_arm_right_angle; lower_arm_left_angle; lower_arm_right_angle; wrist_left_angle; wrist_right_angle
                np.savetxt(data, [angles], fmt='%1.8f', delimiter=';')
